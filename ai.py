...
class Node:
    ...
    def get_successors(self):
        ...
        for swap in get_valid_swaps():
            x1, y1, x2, y2 = swap

            first_board = copy.deepcopy(board)
            dejeweled.swap_gems(first_board, x1, y1, x2, y2)

            first_node = Node(first_board,self.score,0)

            next_successors = [first_node]
            while next_successors != []:
                ...
                for match in matches:
                    for gem in match:
                        next_node.score += (10 + (len(match) - 3) * 10)
